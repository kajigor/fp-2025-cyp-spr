...
spec = do
  describe "Statement parser" $ do
    ...
    it "parses a while statement" $ do
      parseTest "while (x) { print(x); };" `shouldParse` 
        While (VarLit "x") (Write (VarLit "x"))
        
      parseTest "while (i - n) { i = i + 1; };" `shouldParse` 
        While (Sub (VarLit "i") (VarLit "n")) 
              (Assign "i" (Sum (VarLit "i") (IntLit 1)))
              
    it "parses sequence of statements" $ do
      parseTest "a = 1; b = 2;" `shouldParse` 
        Seq (Assign "a" (IntLit 1)) (Assign "b" (IntLit 2))
        
    it "parses nested while loops" $ do
      parseTest "while (x) { while (y) { print(z); }; };" `shouldParse`
        While (VarLit "x") 
              (While (VarLit "y") 
                    (Write (VarLit "z")))
                    
    it "parses complex nested statements" $ do
      parseTest "x = 0; while (x - 10) { print(x); x = x + 1; };" `shouldParse`
        Seq (Assign "x" (IntLit 0))
            (While (Sub (VarLit "x") (IntLit 10))
                  (Seq (Write (VarLit "x"))
                       (Assign "x" (Sum (VarLit "x") (IntLit 1)))))
                       
    it "parses the fibonacci example program" $ do
      let fibProgram = "n = readInt(); \
                       \a = 0; \
                       \b = 1; \
                       \i = 1; \
                       \while ((n - i)) { \
      ...
      parseTest fibProgram `shouldParse` 
        Seq (Read "n")
            (Seq (Assign "a" (IntLit 0))
                 (Seq (Assign "b" (IntLit 1))
                      (Seq (Assign "i" (IntLit 1))
                           (Seq (While (Sub (VarLit "n") (VarLit "i"))
                                       (Seq (Assign "next" (Sum (VarLit "a") (VarLit "b")))
                                            (Seq (Assign "a" (VarLit "b"))
                                                 (Seq (Assign "b" (VarLit "next"))
                                                      (Assign "i" (Sum (VarLit "i") (IntLit 1)))))))
                                (Write (VarLit "b"))))))
    ...
    it "handles newlines after semicolons" $ do
      ...
      parseTest programWithNewlines `shouldParse`
        Seq (Assign "x" (IntLit 1)) 
            (Seq (Assign "y" (IntLit 2))
                 (Assign "z" (IntLit 3)))
                 
    it "handles mixed formatting with multiple newlines" $ do
      let mixedProgram = "a = 10;\n\nb = 20;\n\n\nc = 30;"
      
      parseTest mixedProgram `shouldParse`
        Seq (Assign "a" (IntLit 10))
            (Seq (Assign "b" (IntLit 20))
                 (Assign "c" (IntLit 30)))
